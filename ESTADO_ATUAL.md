...
- **Memória:** `POST /memory/remember` guarda conteúdo novo e substancial, e o que o dono manda explicitamente (`source: "usuário"`). Continua **descartando de propósito** ruído curto ("ok"), saudação e repetição do mesmo texto — é o trabalho do filtro de atenção, não uma falha. *(Correção de atribuição: até aqui este documento dizia que o `stored:false` vinha da falta de chromadb+sentence-transformers. Estava errado — a rejeição acontecia no filtro de atenção, cujo limiar era inalcançável, antes de qualquer store vetorial ser tocado; instalar as libs não mudaria nada.)* As libs opcionais seguem valendo para **qualidade semântica** do recall, não para ele existir.
